...
preliminary_output_revert_options = ["agp", "scaffolding", "skip-gaps", "skip-gaps-file"]
...
additional_lacheck_options = make_flags(["-v"])
...
def LAmerge(merged="output", parts="input[block_alignments]", dbs="params.dbs", log="log"):
    ...
    if log:
        indent = 4 * " "
        full_cmd = indent + lamerge_cmd + "\n" + indent + lacheck_cmd + "\n"
        return "{{ " + full_cmd + " }} &> {" + log + ":q}"
    else:
        return lamerge_cmd + "\n" + lacheck_cmd
...
rule propagate_mask_back_to_reference:
    input:
        dentist_config_file,
        await_db_files(reference),
        lambda wildcards : [propagate_mask_batch_marker.format(blocks_reads=b, mask=wildcards.mask) for b in blocks_for(reads, block_range(propagate_batch_size))],
        mask_files(reference, "{mask}"),
...
rule process:
    ...
    container: dentist_container
    shell:
        "dentist process --config={input.config} {dentist_flags} --threads={params.main_threads} --auxiliary-threads={params.auxiliary_threads} --mask={params.masks} --batch={params.batch_range} {input.refdb[0]} {input.readsdb[0]} '<ignored>' {input.pile_ups} {output} 2> {log}"
...
rule preliminary_output:
    ...
    container: dentist_container
    shell:
        "dentist output --config={input.config} {dentist_flags} --closed-gaps-bed={output[1]} {params.revert_options} {input.refdb[0]} {input.insertions} {output[0]} 2> {log}"
