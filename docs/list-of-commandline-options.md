...
- `--agp-dazzler `: (`output`)  
    use Dazzler DB contig IDs to identify AGP components

- `--agp-skip-read-ids `: (`output`)  
    do not enumerate all reads used for gap closing in AGP file to reduce the resulting file size
...
- `--only <OnlyFlag>(spanning)`: (`process-pile-ups`, `output`)  
    only process/output insertions of the given type. Allowed values are `spanning` (i.e. if two contigs would be properly joined), `extending` (i.e. the sequence just extends the contig but does not reach across two contigs) and `both`. Note, extending insertions are experimental and may produce invalid results.
...
---
<small>This file was generated by `dentist --list-options` at v4.0.0-7-ge830dec.</small>
